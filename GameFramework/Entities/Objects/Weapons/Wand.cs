...
namespace GameFramework.Entities.Objects.Weapons
{
    ...
    {
        public Wand(Position position, bool loot, string name, string symbol, bool block, bool reusable, int damageModifier) : base(position, loot, name, symbol, block, reusable, damageModifier)
        {
        }
        ...
        public override void Use(ref IPlayer creature, List<IWorldObject> objList, Action<IWorldObject> testAction)
        {
            if (creature is Mage)
            {
                creature = new WandDecorated(creature, this);
                ClearItem(objList);
            }
