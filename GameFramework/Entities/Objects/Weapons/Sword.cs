...
namespace GameFramework.Entities.Objects.Weapons
{
    ...
    {
        public Sword(Position position, bool loot, string name, string symbol, bool block, bool reusable, int damageModifier) : base(position, loot, name, symbol, block, reusable, damageModifier)
        {
        }
        ...
        {
            if (creature is Warrior)
            {
                creature = new SwordDecorated(creature,this);
                ClearItem(objList);
            }
